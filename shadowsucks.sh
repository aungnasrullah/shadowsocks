...
cat > ~/client.json <<END
{
    ...
    "password": "taikucing",
    "plugin": "obfs-local",
    "plugin_opts": "obfs=tls;obfs-host=line.store.me"
}
END
# Generate Shadowsocks URI
SS_URI="ss://$(echo -n aes-256-cfb:taikucing | base64 --wrap=0 | tr +/ -_)@$SERVER_IP:1443/?plugin=obfs-local%3bobfs%3dtls%3bobfs-host%3dline.store.me"
echo $SS_URI > ~/ss-uri.txt
# Show config to user.
echo "Hello boss, your Shadowsock server is ready to use!"
...
echo "Enjoy! This script mod by Aung Nasrullah"
